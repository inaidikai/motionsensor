...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  recordKeypointsBtnElem.addEventListener("click", () => {
    ...
    if (WOPose.isExtractKeypoints) {
      pingRecordElem.classList.remove("bg-white-500");
      ...
      pingRecordElem.classList.add("bg-white-500");
        ...
        >
          <div class="flex flex-col items-start justify-between">
            <div class="flex flex-row items-center">
              ...
              <div
                class="text-xs px-1 py-0.5 bg-white-200 rounded-lg text-gray-600 font-semibold"
              >
                ${data.duration}
              </div>
            </div>
  ...
  uploadVideoBtnElem.addEventListener("change", (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      newWebcamElem.setAttribute("class", "bg-white-200 z-10");
